...
\begin{proof}
  ...
  $$
  u = u_0 + \nabla \pi \qquad\text{mit}\qquad u_0 \in \Ell_\sigma^p(\Omega), \nabla\pi \in \GG_p(\Omega).
  $$
...
\begin{proof}
  ...
  Der weitere Beweis gliedert sich in 3 Schritte.
  \begin{enumerate}[(i)]
...
\item Zeige nun, dass $(\Ell_\sigma^{p}(\Omega))' \simeq \Ell^{p'}_\sigma(\Omega)$.
  ...
  $$
  Dies liefert $(I - \PP_{p'}) \tilde f = 0$, woraus wiederum $\tilde f \in \Ell_\sigma^{p'}(\Omega)$ folgt.\qedhere
  \end{enumerate}
\end{proof}
...
\begin{proof}
  \begin{enumerate}[i)]
    ...
    \item Gagliardo-Nirenberg \ref{hsatz:gagliardoNirenberg} mit $p = q$, $r = p$, $q = p$, $j = 0$ und $m = 1$ liefert
      ...
      Anwendung des Halbgruppengesetzes ergibt
      \begin{align*}
        \|\e^{-tA_p} a\|_{\Ell^q} 
