...
\begin{thm}
  \label{thm:bogowskii}
  Sei $\Omega \subset \R^d, d \geq 2$, ein beschränktes Lipschitz-Gebiet, dann existiert ein Operator $\BB$, sodass für jedes $1 < p < \infty$ gilt:
  \begin{align*}
    &\BB \colon \Ell_0^p(\Omega) \to \WW_0^{1,p}(\Omega, \C^d), \quad \BB \in \Li\big(\Ell_0^p(\Omega), \WW_0^{1,p}(\Omega, \C^d)\big) \\
...
\begin{proof}
  Sei $\BB$ der Bogowski\u{\i}-Operator aus Satz \ref{thm:bogowskii} und $f \in \Ell^{p'}(\Omega)$, wobei $\frac{1}{p} + \frac{1}{p'} = 1$.
  Dann gilt
  \begin{align*}
